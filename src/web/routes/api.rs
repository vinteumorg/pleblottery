...
pub async fn get_current_height(State(shared_state): State<SharedStateHandle>) -> Html<String> {
    let state = shared_state.read().await;
    if let Some(current_height) = &state.latest_template {
        let current_height = current_height.coinbase_prefix.to_vec().as_slice()[1..]
            .iter()
            .rev()
...
pub fn api_routes(shared_state: SharedStateHandle) -> Router {
    Router::new()
        ...
        .route(
            "/api/current-height",
            axum::routing::get(get_current_height),
        )
